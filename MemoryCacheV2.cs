...
namespace LiteDB.Engine
{
    ...
    internal class MemoryCacheV2 : IDisposable
    {
        ...
        /// <summary>
        /// Ensures thread-safe access to critical sections of the Page management logic,
        /// preventing race conditions in multi-threaded scenarios.
        /// </summary>
        private static readonly SemaphoreSlim _semaphoreSlim = new SemaphoreSlim(1, 1);

        public MemoryCacheV2(int[] memorySegmentSizes, MemoryCacheConfig config)
        {
            _segmentSizes = memorySegmentSizes;
            _bufferManager = new PageBufferManager(config, _free);
            Extend();
        }
        ...
        private void DiscardPageInternal(PageBuffer page)
        {
            ...
            FreePage(page, 0, long.MaxValue, FileOrigin.None);
        }
        ...
        private PageBuffer GetFreePage()
        {
            while (true)
            {
                ...
                {
                    // Validamos que la página sea válida
        ...
        private void Extend()
        {
            ...
            if (emptyShareCounter > segmentSize)
            {
                ...
                foreach (var key in readables)
                {
                    ...
                    else
                    {
                        ...
                        FreePage(page, null, long.MaxValue, FileOrigin.None);
                    }
                }
            ...
            else
            {
                ...
                {
                    var newPage = new PageBuffer(buffer, i * PAGE_SIZE, uniqueID++);
                    FreePage(newPage);
                }
            ...
            System.Diagnostics.Debug.WriteLine($"_readable.Count: {_readable.Count}");
        }
        ...
        private void FreePage(PageBuffer page, int? shareCounter = null, long? position = null, FileOrigin? origin = null)
        {
            if (shareCounter.HasValue)
            {
                page.ShareCounter = shareCounter.Value;
            }

            if (position.HasValue)
            {
                page.Position = position.Value;
            }

            if (origin.HasValue)
            {
                page.Origin = origin.Value;
            }

            page.TimestampFree = DateTime.UtcNow.Ticks;
